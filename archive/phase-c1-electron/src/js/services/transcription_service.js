...
class TranscriptionService {
   ...
   * @param {string} baseDir - Base directory path for resolving Python paths
   */
  constructor(mainWindow, baseDir) {
    this.mainWindow = mainWindow;
    this.baseDir = baseDir;
    this.pythonPath = path.join(baseDir, config.get('paths.pythonVenv'));
    this.scriptPath = path.join(baseDir, config.get('paths.transcriberScript'));
  }
   ...
   *
   * @param {string} audioPath - Absolute path to audio file to transcribe
   * @returns {Promise<string>} Resolves with transcript path, rejects on error
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        transcriber.stdout.on('data', (data) => {
          ...
          if (output.startsWith(config.get('protocol.transcriptSaved'))) {
   ...
   *
   * @param {string} channel - IPC channel name (e.g., 'transcription-started')
   * @param {*} [data=null] - Optional data payload to send
   * @private
   * @example
   * this.notifyUI('transcription-complete');
