 ...
 * @returns {string} Duration string like "11 sec" or "2m 30s"
 */
function getAudioDuration(audioPath) {
  try {
    const stats = fs.statSync(audioPath);
    // Rough estimate: 44100 Hz * 2 bytes * 1 channel ≈ 88200 bytes/sec
    // Add header size (~44 bytes)
    const durationSec = Math.max(0, (stats.size - 44) / 88200);
...
function viewTranscript(transcriptPath) {
  ipcRenderer.send('view-file', transcriptPath);
}
...
async function copyTranscript(transcriptPath) {
  try {
    const content = await ipcRenderer.invoke('read-file', transcriptPath);
